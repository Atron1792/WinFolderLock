...
> **Note:** This program is intended for **convenience** rather than strict, enterprise-style security. It provides an easy way to password-protect folders from casual access without having to setup multiple users on Windows.

A simple utility to protect your folders on Windows using the `.wflck` extension and AES-256-GCM encryption. WinFolderLock integrates directly with the Windows context menu, allowing you to easily lock and unlock folders.
...
- AES-256-GCM Encryption standard for securing data reliably.
- DPAPI Protection ensures the encryption key is tightly bound to the current Windows user profile.
...
Run the `WinFolderLock.exe` without any arguments to open the installation wizard.
...
To uninstall run the wizard again, it will prompt you for the uninstallation and permanently unlock all locked folders properly doing its cleanup.
